...
function PromotionCanvas() {
      ...
      <div className="card p-1 mb-2" >
        <div className="row g-1">
          <div className="col-md-4">
            <img src={datapromotion.imageurl} className="img-fluid rounded-start" style={{width: 100 +'%',height: 100 +'%'}} alt="..."/>
          </div>
          <div className="col-md-8">
            <div className="card-body">
              <h6 className="card-title font-monospace fw-bold border-bottom border-1 border-danger py-1">{datapromotion.title}</h6>
              <p className="card-text"><small>{datapromotion.description}</small></p>
              <span className='float-end'style={{cursor: 'pointer'}} onClick={()=>{onPromotionClick(datapromotion)}}><small className="card-text text-primary" >รายละเอียด....</small></span>
            </div>
          </div>
        </div>
      </div>
  ...
  <div onClick={onBgClick} className='bg-transparent position-fixed top-0 start-0' style={{width: 100 + 'vw', height: 100 + 'vh', backdropFilter: "blur( 8px)"}}>
    ...
    if (!!postpromotion) {
      promotionPost = <PromotionPost promotion={postpromotion} onBgClick={onPromotionClose} />
    }
  ...
  return (
    <div className='d-flex justify-content-center'>
      ...
      <Offcanvas show={show} placement='start' onHide={handleClose} scroll={false} backdrop={false}  style={{zIndex: 9999}}>
